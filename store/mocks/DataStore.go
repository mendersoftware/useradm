...
// GetUserForLogin provides a mock function with given fields: ctx, email, loginTS
func (_m *DataStore) GetUserForLogin(ctx context.Context, email string, loginTS time.Time) (*model.User, error) {
	ret := _m.Called(ctx, email, loginTS)

	var r0 *model.User
	if rf, ok := ret.Get(0).(func(context.Context, string, time.Time) *model.User); ok {
		r0 = rf(ctx, email, loginTS)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.User)
		}
